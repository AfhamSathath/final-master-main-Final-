...
const Register: React.FC = () => {
  ...
  const verifyCompany = async () => {
    if (!formData.name) {
      toast.error("⚠️ Please provide a company name first!");
      return;
    }

    try {
      const toastId = toast.loading("🔍 Verifying company authenticity...");
      const res = await axios.post(`${API_BASE}/api/verify-company`, {
      ...
      const reason = data.reason || (isVerified ? "Verified" : "Not verified");
      ...
      setVerified(false);
      setConfidence(null);
      console.error("verifyCompany error:", err?.response?.data || err);
    }
  };
