...
const registerValidationSchema = z.object({
  ...
  regNumber: z.string().optional(), // Will validate conditionally below
});
...
const Register: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Conditional regNumber validation for company
      if (formData.userType === "company" && (!formData.regNumber || formData.regNumber.trim().length < 3)) {
        setErrorMsg("Registration number must be at least 3 characters");
        toast.error("⚠️ Registration number must be at least 3 characters");
        setLoading(false);
        return;
      }
