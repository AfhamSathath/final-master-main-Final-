...
import { getUser } from "@/utils/Auth"; // ✅ get logged-in company info
...
type Course = {
  ...
  name: string;
  description: string;
  institution: string; // Company name here
...
const CompanyCoursesPage: React.FC = () => {
  ...
  const user = getUser(); // ✅ get logged-in company
  ...
  const [formData, setFormData] = useState<NewCourse>({
    name: "",
    description: "",
    institution: companyName, // ✅ auto-fill with logged-in company
    qualification: "",
    duration: "",
    category: "",
  });
  ...
  // ✅ Keep company name fixed (auto-reset if someone edits)
  useEffect(() => {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setFormData({
        name: "",
        description: "",
        institution: companyName, // ✅ reset to company
        qualification: "",
        duration: "",
        category: "",
      });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const payload = { ...formData, institution: companyName }; // ✅ always enforce company name
  ...
  // ================== FILTERED COURSES ==================
  const companyCourses = courses.filter(
    (course) => course.institution === companyName // ✅ only show logged-in company courses
  );

  const filteredCourses = filterCategory
    ? companyCourses.filter((course) => course.category === filterCategory)
    : companyCourses;
  ...
  return (
    <div className="p-8 bg-gray-50 min-h-screen">
      ...
      {filteredCourses.length === 0 ? (
        <p className="text-center text-gray-600">No courses found for your company.</p>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredCourses.map((course) => (
            ...
            >
              ...
              <p className="text-gray-700 mb-1">Qualification: {course.qualification || "N/A"}</p>
              <p className="text-gray-500 mb-2">Duration: {course.duration || "N/A"}</p>
              <p className="text-gray-700 text-sm mb-4">{course.description || "No description."}</p>
                ...
                <button
                  onClick={() => {
                    ...
                    setFormData({
                      ...
                      institution: companyName, // ✅ keep company fixed
