...
router.post("/forgot-password", async (req, res) => {
  try {
    ...
    }else if(!Company.findOne({userId: user._id, isVerified: true})){
      return res.status(400).json({ success: false, message: "Email is not verified." });
    }else if(user.authType !== 'local'){
      return res.status(400).json({ success: false, message: `You have registered using ${user.authType}. Please use ${user.authType} login to access your account.` });
    }
